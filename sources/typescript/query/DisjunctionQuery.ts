...
export class DisjunctionQuery extends BasicQuery<boolean> {
  /**
  * Operands of this disjunction.
  */
  ...
  */
  public constructor (operands? : Iterable<UnidocQuery<boolean>>) {
    ...
    if (operands) {
      for (const operand of operands) {
        if (operand instanceof DisjunctionQuery) {
  ...
  /**
  * Return a new disjunction that also contains the given operators.
  *
  * If one of the given operators is also a disjunction, both disjunctions will
  ...
  * @return A new disjunction that also contains the given operators.
  */
  public or (...operands : UnidocQuery<boolean>[]) : DisjunctionQuery {
    ...
    return new DisjunctionQuery(result)
  }
    ...
    for (let index = 0, size = this.operands.length; index < size; ++index) {
      ...
      switch (buffer.first) {
        case TRUE:
          ...
          break
        case FALSE:
          state = state === COMPLETE ? FALSE : state
          buffer.shift()
          break
        default:
          break
  ...
  */
  public clone () : DisjunctionQuery {
    const result : DisjunctionQuery = new DisjunctionQuery(this.operands.map(x => x.clone()))
