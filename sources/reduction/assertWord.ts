 ...
 *
 */
export function* assertWord(): UnidocReduction<void> {
  ...
  if (!current.value.isWord()) {
    throw new Error(`${current.value.stringifyLocation()} : expected to receive word, but received ${current.value.stringifyContent()}.`)
  }
}