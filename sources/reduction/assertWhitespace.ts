 ...
 *
 */
export function* assertWhitespace(): UnidocReduction<void> {
  ...
  if (!current.value.isWhitespace()) {
    throw new Error(`${current.value.stringifyLocation()} : expected to receive whitespace, but received ${current.value.stringifyContent()}.`)
  }
}